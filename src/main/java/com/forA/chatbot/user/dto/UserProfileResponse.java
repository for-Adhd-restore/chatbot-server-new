...
public class UserProfileResponse {
    ...
    public UserProfileResponse(User user) {
        ...
        this.job = user.getJobs().stream()
                .map(JobType::getName)
                .collect(Collectors.toList());
        this.mentalConditions = user.getDisorders().stream()
                .map(DisorderType::getName)
                .collect(Collectors.toList());
        this.symptoms = user.getSymptoms().stream()
                .collect(Collectors.groupingBy(
                        symptom -> symptom.getDisorderType().getName(),
                        Collectors.mapping(
                                SymptomType::getDescription,
                                Collectors.toList()
                        )
                ));
    }
