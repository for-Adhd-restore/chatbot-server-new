...
public class UserDeletionScheduler {
  ...
  public void deleteExpiredUsers() {
    ...
    for (User user : usersToDelete) {
      try {
        ...
        deletedCount++;
        log.info(
            "Permanently deleted user account: userId={}, deletedAt={}",
            user.getId(),
            user.getDeletedAt());

      } catch (Exception e) {
        log.error(
            "Failed to delete user account: userId={}, error={}", user.getId(), e.getMessage(), e);
      }
    }
  ...
  public void logPendingDeletions() {
    ...
    log.info(
        "User deletion statistics - Expired (ready for deletion): {}, "
            + "Will expire in 7 days: {}",
        expiredUsers.size(),
        soonToBeDeleted.size() - expiredUsers.size());
  }
}
